...
// MasterChef is the master of Inc. He can make Inc and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once INCUM is sufficiently
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of INCs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. INCs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that INCs distribution occurs.
        uint256 accIncPerShare;   // Accumulated INCs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    uint256 public constant incMaximumSupply = 100 * (10 ** 3) * (10 ** 18);

    uint256 public constant incPreMint = 375 * (10 ** 2) * (10 ** 18);

    // The INCUM TOKEN!
    IncToken public inc;
    // INCUM tokens created per block.
    uint256 public incPerBlock;
    // Deposit Fee address
    ...
    // The block number when INCUM mining starts.
    uint256 public startBlock;
    // The block number when INCUM mining ends.
    ...
    constructor(
        IncToken _inc,
        address _feeAddress,
        uint256 _incPerBlock,
        uint256 _startBlock
    ) Ownable(msg.sender) {
        inc = _inc;
        feeAddress = _feeAddress;
        incPerBlock = _incPerBlock;
    ...
    // Update the given pool's INCUM allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending INCs on frontend.
    function pendingInc(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0) {
            ...
            accIncPerShare = accIncPerShare + ((incReward * 1e12) / pool.lpSupply);
        }

        return ((user.amount * accIncPerShare) /  1e12) - user.rewardDebt;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (inc.totalSupply() > incMaximumSupply)
            incReward = 0;
        else if ((inc.totalSupply() + incReward) > incMaximumSupply)
            incReward = incMaximumSupply - inc.totalSupply();

        if (incReward > 0)
            inc.mint(address(this), incReward);

        // The first time we reach incums max supply we solidify the end of farming.
        if (inc.totalSupply() >= incMaximumSupply && emmissionEndBlock == type(uint256).max)
            emmissionEndBlock = block.number;

        pool.accIncPerShare = pool.accIncPerShare + ((incReward * 1e12) / pool.lpSupply);
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for INCUM allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = (user.amount * pool.accIncPerShare) / 1e12;
    ...
    // Safe inc transfer function, just in case if rounding error causes pool to not have enough INCs.
    function safeIncTransfer(address _to, uint256 _amount) internal {
        uint256 incBal = inc.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > incBal) {
            transferSuccess = inc.transfer(_to, incBal);
        } else {
            transferSuccess = inc.transfer(_to, _amount);
        }
        require(transferSuccess, "safeIncTransfer: transfer failed");
    }
